...
Hệ thống trạm quan trắc chất lượng không khí đa thông số được xây dựng theo mô hình IoT ba tầng: tầng nhận thức (Perception Layer), tầng truyền dẫn (Network Layer) và tầng ứng dụng (Application Layer). Kiến trúc tổng quan của hệ thống được minh họa như sau:

\begin{figure}[H]
    \centering
    % \includegraphics[width=0.9\textwidth]{images/system_overview.png}
    \textbf{[CHÈN SƠ ĐỒ CẤU TRÚC TỔNG QUAN HỆ THỐNG VÀO ĐÂY]}
...
\begin{itemize}
    \item \textbf{Khối thu thập dữ liệu (Embedded Node):} Vi điều khiển ESP32 tích hợp WiFi đọc đồng thời ba luồng dữ liệu từ cảm biến bụi mịn PMS5003 (UART), cảm biến vi khí hậu BME680 (I2C) và cảm biến khí độc MQ-135 (ADC). Dữ liệu được lọc số, tính chỉ số AQI rồi đóng gói JSON và đẩy lên broker MQTT.
    \item \textbf{Khối truyền dẫn (MQTT Broker):} Giao thức MQTT qua chuẩn mạng WiFi 802.11n giúp truyền tải dữ liệu nhẹ, ổn định qua Internet tới broker công cộng \texttt{broker.emqx.io}.
    \item \textbf{Khối xử lý và lưu trữ (Backend Cloud):} Dịch vụ Node.js đăng ký (subscribe) topic MQTT, nhận gói tin và lưu vào cơ sở dữ liệu MongoDB Atlas trên đám mây.
    \item \textbf{Khối hiển thị (Web Dashboard):} Ứng dụng web Next.js triển khai trên Vercel cung cấp giao diện giám sát theo thời gian thực, hiển thị biểu đồ lịch sử và bản đồ vị trí trạm. Người dùng có thể theo dõi từ xa thông qua trình duyệt trên điện thoại, máy tính.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/flowchart_uart_parsing.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/block_dual_core.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/flowchart_wdt.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/flowchart_wifi.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{images/flowchart_main.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{images/flowchart_aqi.png}
...
Thiết bị phần cứng hoàn chỉnh được gia công trên mạch in PCB hai lớp chống nhiễu, bao bọc trong hộp bảo vệ nhựa ABS có khe thông gió tự nhiên để đảm bảo luồng không khí lưu thông tối ưu qua các cảm biến.
