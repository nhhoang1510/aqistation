...
\hline
 & \centering\arraybackslash\includegraphics[width=0.28]{images/esp32.png} \\ 
        \hline
...
Hệ thống Web Dashboard được xây dựng bằng Next.js, triển khai trên Vercel, là giao diện giám sát chất lượng không khí thời gian thực dành cho người dùng cuối. Dashboard có 4 trang chức năng được điều hướng qua thanh Sidebar bên trái: \textbf{Trang chủ}, \textbf{Lịch sử dữ liệu}, \textbf{Bản đồ trạm} và \textbf{Cài đặt cảnh báo}.

\subsubsection{Trang chủ -- Hiển thị chỉ số AQI và thông số thời gian thực}

Trang chủ hiển thị tiêu đề \textit{``TRẠM QUAN TRẮC CHẤT LƯỢNG KHÔNG KHÍ ĐA THÔNG SỐ''} cùng badge nhận diện \textit{TRẠM 01} và chỉ báo trạng thái \textbf{● Trực tuyến} ở góc phải. Dòng thời gian cập nhật lần cuối được hiển thị ngay bên dưới tiêu đề (ví dụ: \textit{Cập nhật lúc: 17:30:37 14/07/2026}).

Phần trung tâm trang chủ gồm hai thành phần chính:
\begin{itemize}
    \item \textbf{Chỉ số AQI hiện tại:} Giá trị AQI được hiển thị cỡ chữ lớn nổi bật (ví dụ: \textbf{114}) kèm nhãn phân loại màu (ví dụ: \textit{Kém}). Bên dưới là thanh màu gradient từ xanh lá đến đỏ sẫm (từ 0 đến 300+) theo đúng thang chuẩn US EPA, với mũi tên đánh dấu vị trí AQI hiện tại.
    \item \textbf{4 thẻ thông số phụ:} Hiển thị song song các chỉ số môi trường cốt lõi: PM2.5 (µg/m³), PM10 (µg/m³), Nhiệt độ (°C) và Độ ẩm (\%).
\end{itemize}
...
\begin{figure}[H]
    ...
    \textit{[Hình ảnh trang chủ Dashboard: AQI = 114 ``Kém'', thanh màu gradient, 4 thẻ PM2.5=40.8 µg/m³, PM10=49.8 µg/m³, Nhiệt độ=32°C, Độ ẩm=65\%]}
    \vspace{2.5cm}}}
    \caption{Trang chủ Dashboard hiển thị AQI hiện tại và 4 thông số môi trường chính}
    \label{fig:dashboard_home}
\end{figure}

\subsubsection{Biểu đồ theo thời gian -- 6 thông số song song}

Phần dưới trang chủ hiển thị \textbf{6 biểu đồ đường} (Line Chart) vẽ song song theo thời gian thực cho toàn bộ các thông số cảm biến: PM2.5, PM10, Nhiệt độ, Độ ẩm, Áp suất và Gas VOC. Mỗi biểu đồ được tô màu nền (area fill) riêng biệt và có thể tùy chọn khung thời gian bằng menu thả xuống (1 phút, 5 phút, 1 giờ, 24 giờ).

Các giá trị thực tế ghi nhận trong khoảng thời gian từ 17:30 đến 17:32:
\begin{itemize}
    \item PM2.5: dao động 39,5 -- 42,0 $\mu$g/m³
    \item PM10: dao động 48,0 -- 51,0 $\mu$g/m³
    \item Nhiệt độ: dao động 31,50 -- 31,60 °C
    \item Độ ẩm: dao động 64,70 -- 64,80 \%
    \item Áp suất: dao động 998,80 -- 998,90 hPa
    \item Gas VOC: dao động 22,65 -- 22,90 (đơn vị nội bộ)
\end{itemize}
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.95\textwidth}{\centering\vspace{2.5cm}
    \textit{[Hình ảnh 6 biểu đồ thời gian thực: PM2.5 (hồng), PM10 (cam), Nhiệt độ (đỏ), Độ ẩm (xanh dương), Áp suất (tím), Gas VOC (xanh lá)]}
    \vspace{2.5cm}}}
    \caption{6 biểu đồ đường diễn biến thời gian thực của các thông số môi trường}
    \label{fig:dashboard_charts}
\end{figure}

\subsubsection{Trang Lịch sử dữ liệu}

Trang \textbf{Lịch sử dữ liệu} cung cấp công cụ phân tích chuyên sâu với bộ lọc theo khoảng ngày (\textit{Từ ngày} -- \textit{Đến ngày}), nút \textit{Tìm kiếm} và nút \textit{Xuất CSV} để tải dữ liệu về phân tích ngoại tuyến.

\textbf{Khối thống kê tổng hợp} hiển thị 4 chỉ số thống kê của khoảng thời gian đã chọn:
\begin{itemize}
    \item \textbf{Tổng bản ghi:} 1.334 bản ghi (ngày 14/07/2026)
    \item \textbf{AQI trung bình:} 61,7 -- phân loại \textit{Trung bình}
    \item \textbf{AQI cao nhất:} 126 -- phân loại \textit{Kém}
    \item \textbf{PM2.5 trung bình:} 21,6 $\mu$g/m³
\end{itemize}

Bên dưới là \textbf{biểu đồ đường AQI} toàn ngày và bảng dữ liệu chi tiết gồm 8 cột: Thời gian, AQI, PM2.5, PM10, Nhiệt độ, Độ ẩm, Áp suất, Gas VOC, MQ-135. Các giá trị AQI trong bảng được hiển thị dạng badge màu sắc theo chuẩn US EPA.

\begin{figure}[H]
    \centering
    % \includegraphics[width=1.0\textwidth]{images/history_page.png}
    \fbox{\parbox{0.95\textwidth}{\centering\vspace{2.5cm}
    \textit{[Hình ảnh trang Lịch sử: Bộ lọc ngày, 4 thẻ thống kê (1334 bản ghi, AQI TB=61.7, AQI max=126, PM2.5 TB=21.6 µg/m³), biểu đồ AQI toàn ngày]}
    \vspace{2.5cm}}}
    \caption{Trang Lịch sử dữ liệu với thống kê tổng hợp và biểu đồ AQI theo ngày}
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.95\textwidth}{\centering\vspace{2.5cm}
    \textit{[Hình ảnh bảng dữ liệu lịch sử: 8 cột Thời gian -- AQI -- PM2.5 -- PM10 -- Nhiệt độ -- Độ ẩm -- Áp suất -- Gas VOC -- MQ-135, badge AQI màu xanh lá]}
    \vspace{2.5cm}}}
    \caption{Bảng dữ liệu chi tiết lịch sử với 8 thông số đo và badge AQI màu sắc}
    \label{fig:history_table}
\end{figure}

\subsubsection{Trang Cài đặt cảnh báo}

Trang \textbf{Cài đặt cảnh báo} cho phép người dùng cá nhân hóa hoàn toàn chế độ thông báo:
\begin{itemize}
    \item \textbf{Bật/Tắt cảnh báo:} Toggle ``Bật cảnh báo'' -- Tự động gửi thông báo khi AQI vượt ngưỡng.
    \item \textbf{Thông tin liên hệ:} Nhập địa chỉ email và số điện thoại nhận thông báo.
    \item \textbf{Thông báo trực tiếp:} Nút ``Bật Push'' kích hoạt Push Notification ngay trên trình duyệt.
    \item \textbf{Ngưỡng AQI:} Thanh trượt điều chỉnh từ 30 đến 300, có sẵn 4 nút tắt nhanh: 50 (Tốt), 100 (TB), 150 (Kém), 200 (Xấu). Ngưỡng hiện tại mặc định là 90 (Trung bình).
    \item \textbf{Thời gian chờ:} Khoảng cách tối thiểu giữa hai lần gửi cảnh báo, chọn trong 4 mức: 15 phút, 30 phút, 1 giờ, 3 giờ.
    \item \textbf{Lần gửi gần nhất:} Hiển thị timestamp lần gửi cảnh báo gần nhất (ví dụ: 12:55:49 14/7/2026).
\end{itemize}
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.95\textwidth}{\centering\vspace{2.5cm}
    \textit{[Hình ảnh trang Cài đặt cảnh báo: Toggle bật cảnh báo, form email/SĐT, nút Bật Push, thanh trượt ngưỡng AQI=90, thời gian chờ 30 phút, nút Lưu cài đặt]}
    \vspace{2.5cm}}}
    \caption{Trang Cài đặt cảnh báo với ngưỡng AQI tùy chỉnh và thời gian chờ giữa các lần gửi}
