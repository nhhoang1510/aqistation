...
\subsubsection{Quy trình thiết kế và phát triển Web Dashboard}
Nhằm đảm bảo giao diện giám sát (Web Dashboard) đáp ứng đầy đủ các tiêu chí: trực quan, thời gian thực, có khả năng mở rộng và mang lại trải nghiệm người dùng tối ưu, quy trình thiết kế và phát triển phần mềm giao diện đã được tuân thủ nghiêm ngặt theo mô hình linh hoạt kết hợp với các bước kỹ nghệ phần mềm tiêu chuẩn. Quá trình này được chia thành 5 bước cụ thể và chi tiết như sau:

\paragraph{Bước 1: Phân tích yêu cầu và Lên ý tưởng (Ideation \& Requirements)}\mbox{}\\
...
\begin{itemize}
    ...
    \item \textbf{Yêu cầu phi chức năng:} Độ trễ cập nhật dữ liệu phải thấp (dưới 10 giây). Giao diện phải hoạt động mượt mà trên cả trình duyệt máy tính lẫn thiết bị di động (Responsive Design).
\end{itemize}

\paragraph{Bước 2: Lựa chọn công nghệ (Tech Stack Selection)}\mbox{}\\
Sau khi xác định yêu cầu, nhóm tiến hành đánh giá và lựa chọn các công nghệ phù hợp:
\begin{itemize}
    \item \textbf{Frontend Framework:} Next.js (dựa trên React) được lựa chọn nhờ khả năng kết hợp hoàn hảo giữa Server-Side Rendering (SSR) và Client-Side Rendering (CSR), giúp tăng tốc độ tải trang ban đầu và tối ưu hóa hệ thống.
    \item \textbf{Styling:} Tailwind CSS được sử dụng để xây dựng giao diện. Hệ thống lớp tiện ích (utility-first) của Tailwind cho phép tùy biến linh hoạt, đồng nhất về thiết kế (Design System) và hỗ trợ Mobile-first dễ dàng mà không cần phải viết các tập tin CSS cồng kềnh.
    \item \textbf{Quản lý trạng thái và Biểu đồ:} React Hooks (\texttt{useState}, \texttt{useEffect}) được dùng để quản lý trạng thái dữ liệu thay đổi liên tục. Thư viện \texttt{Chart.js} kết hợp \texttt{react-chartjs-2} được sử dụng để trực quan hóa dữ liệu lịch sử thành các đồ thị đường (Line Chart).
...
\paragraph{Bước 3: Thiết kế UI/UX và Kiến trúc hệ thống}\mbox{}\\
Giao diện được thiết kế theo trường phái tối giản (Minimalism) và dạng thẻ (Card-based Layout).
\begin{itemize}
    \item \textbf{Bố cục trang tổng quan (Dashboard):} Chia thành hai phần chính. Phần đầu ở trên cùng chứa tên trạm, trạng thái kết nối và khối tính toán AQI chính, tích hợp thanh trượt màu sắc theo chuẩn US EPA. Phần dưới hiển thị các thông số phụ chia theo dạng lưới (Grid 2 cột trên điện thoại, 4 cột trên máy tính bàn).
    \item \textbf{Thanh điều hướng (Sidebar):} Hệ thống sử dụng một Sidebar gắn lề trái màn hình, hỗ trợ thu gọn hoặc bung rộng, giúp người dùng dễ dàng chuyển đổi giữa các module: Dashboard, Lịch sử (History), Bản đồ (Map) và Cài đặt (Settings).
    \item \textbf{Thiết kế luồng dữ liệu (Data Flow):} Trình duyệt định kỳ mỗi 5 giây gửi một HTTP GET Request tới REST API \texttt{/api/data/latest} của Next.js Route Handler để lấy dữ liệu từ MongoDB, sau đó cập nhật trực tiếp vào State của React để kích hoạt quá trình Re-render giao diện tự động (Auto-Refresh).
\end{itemize}

\paragraph{Bước 4: Triển khai lập trình phần mềm (Implementation)}\mbox{}\\
Quá trình lập trình được chia nhỏ thành các thành phần độc lập (Component-driven):
\begin{itemize}
    \item \textbf{Xây dựng các Component dùng chung:} Lập trình các nút bấm, thẻ dữ liệu (Card), và khối biểu đồ để tái sử dụng xuyên suốt mã nguồn.
    \item \textbf{Lập trình Trang Đăng nhập/Đăng ký:} Tích hợp thư viện \texttt{bcryptjs} ở Backend để băm mật khẩu người dùng trước khi ghi vào cơ sở dữ liệu. Thiết lập cảnh báo và ràng buộc các thông báo lỗi trên Form (ví dụ: Mật khẩu quá ngắn, yêu cầu ít nhất 6 ký tự) bằng công cụ HTML5 Validation để bảo vệ an toàn cho dữ liệu đầu vào.
    \item \textbf{Tích hợp API và Vẽ biểu đồ:} Thiết kế thuật toán chuyển đổi mảng dữ liệu. Chuyển đổi nhãn thời gian từ định dạng ISO thành dạng ``giờ:phút'' cục bộ để nạp vào mảng tọa độ trục X của Chart.js.
    \item \textbf{Tích hợp Bản đồ Leaflet:} Xử lý vấn đề đóng gói CSS của Leaflet trong môi trường Next.js. Để vượt qua lỗi Render phía máy chủ (SSR) của React đối với các API liên quan đến \texttt{window}, nhóm đã sử dụng tính năng \texttt{next/dynamic} để chỉ nạp bản đồ phía Client, đồng thời tiêm trực tiếp link CDN của Leaflet vào thẻ \texttt{<head>}, đảm bảo bản đồ hiển thị rõ ràng, mượt mà khi đưa lên máy chủ.
\end{itemize}

\paragraph{Bước 5: Kiểm thử và Triển khai Đám mây (Testing \& Deployment)}\mbox{}\\
\begin{itemize}
    \item \textbf{Kiểm thử cục bộ (Local Testing):} Chạy hệ thống ở môi trường phát triển (\texttt{localhost}), giả lập luồng gói tin MQTT liên tục gửi từ ESP32 để đánh giá tốc độ cập nhật giao diện người dùng và kiểm tra tính đúng đắn của các biểu tượng màu sắc đánh giá AQI.
    \item \textbf{Tối ưu hóa hiệu năng (Optimization):} Giảm thiểu tối đa độ trễ bằng cách tách các module nặng (như biểu đồ và bản đồ) ra khỏi chu kỳ làm mới nếu không có sự thay đổi. Các cờ tải (loading flag) được bổ sung để hiển thị hiệu ứng chờ trực quan trong quá trình truy xuất từ cơ sở dữ liệu.
    \item \textbf{Triển khai Tự động hóa (Cloud Deployment):} Toàn bộ mã nguồn giao diện frontend và các API phụ trợ được lưu trữ tại GitHub. Nền tảng Vercel được liên kết trực tiếp để cấu hình quy trình triển khai liên tục (CI/CD). Mỗi khi có cập nhật, Vercel tự động biên dịch ứng dụng bằng Turbopack và phân phối lên các nút mạng Edge toàn cầu, đảm bảo hệ thống có khả năng mở rộng tự động (auto-scaling) và phục vụ 24/7 mà không cần đến máy chủ vật lý truyền thống.
\end{itemize}
...
Web Dashboard được thiết kế theo mô hình Single Page Application (SPA) kết hợp thanh điều hướng bên (Sidebar Navigation) mượt mà, bao gồm các trang chức năng và thành phần chính sau:

\begin{itemize}
    \item \textbf{Trang Tổng quan (Dashboard):} Hiển thị toàn cảnh dữ liệu thời gian thực.
    \begin{itemize}
        \item \textit{Thanh trạng thái hệ thống}: Hiển thị thông tin tên trạm (Trạm 01), thời điểm cập nhật dữ liệu gần nhất, cường độ tín hiệu mạng WiFi (RSSI) và thời gian hoạt động liên tục (Uptime) của thiết bị nhúng. Hệ thống sử dụng hiệu ứng chớp tắt (pulse) kết hợp chỉ báo Trực tuyến/Ngoại tuyến để thông báo tình trạng kết nối.
        \item \textit{Bảng chỉ số AQI trung tâm}: Cung cấp giá trị AQI tổng hợp nổi bật kèm thanh trượt dải màu tuyến tính. Điểm đánh dấu (marker) trên thanh trượt chỉ định chính xác vị trí của mức AQI hiện tại theo thang chuẩn US EPA, kèm theo nhãn phân loại tự động (Tốt, Trung bình, Kém, Xấu, Rất xấu, Nguy hại).
        \item \textit{Khối thẻ thông số phụ}: Bốn thẻ số liệu nổi bật hiển thị các thông số cốt lõi (PM2.5, PM10, Nhiệt độ, Độ ẩm) dưới dạng khối thẻ bo góc tối giản, tập trung vào con số trực quan nhằm tránh gây rối mắt.
    \end{itemize}
    \item \textbf{Trang Bản đồ trực quan (Map):} Tích hợp bản đồ số thông qua thư viện Leaflet. Nhằm nâng cao trải nghiệm người dùng, hệ thống sử dụng lớp nền bản đồ đường phố của Google Maps (Google Maps Tile Layer). Việc sử dụng trực tiếp tile layer tĩnh giúp hệ thống có được giao diện quen thuộc, độ chi tiết cao từ Google mà không vướng mắc giới hạn hay cấu hình phức tạp của Google Cloud API Key. Trạm đo được ghim vị trí cố định (mặc định tại Đại học Bách Khoa Hà Nội), cho phép người dùng nhấp vào điểm ghim để xem Popup dữ liệu quan trắc tức thời.
    \item \textbf{Phân tích Biểu đồ thời gian (History):} Tích hợp trên trang Dashboard và History, hệ thống vẽ đồ thị đường (Line Chart) diễn biến cho 6 thông số: PM2.5, PM10, Nhiệt độ, Độ ẩm, Áp suất và Gas VOC. Biểu đồ sử dụng thư viện Chart.js kết hợp menu thả xuống cho phép tùy chọn khung thời gian xem linh hoạt (từ 1 phút, 5 phút, 1 giờ cho tới 24 giờ).
    \item \textbf{Hệ thống xác thực và Quản lý tài khoản (NextAuth):} Cung cấp cơ chế phân quyền truy cập thông qua Đăng ký/Đăng nhập an toàn. Thông tin người dùng được mã hóa bằng Bcrypt và lưu trên cơ sở dữ liệu MongoDB Atlas (collection \texttt{users}). Biểu mẫu đăng ký được trang bị bộ kiểm tra tính toàn vẹn (Custom HTML5 Validation) nhằm hiển thị các cảnh báo tức thời, thân thiện khi mật khẩu quá ngắn hoặc không đạt độ an toàn cần thiết.
    \item \textbf{Giao diện đáp ứng (Responsive Design):} Toàn bộ ứng dụng web được xây dựng dựa trên framework Tailwind CSS theo chuẩn Mobile-first. Bố cục dạng lưới (grid system) đảm bảo các khối thẻ dữ liệu và biểu đồ tự động co giãn, tự sắp xếp lại một cách mượt mà trên mọi kích thước thiết bị, từ điện thoại di động đến màn hình lớn.
\end{itemize}
